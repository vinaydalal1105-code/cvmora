...
export function BoldTemplate({ data, accentColor }: { data: ResumeData; accentColor?: string }) {
  ...
  const headerSubClass = headerLight ? 'text-[#4b5563]' : 'text-[#e5e7eb]'
  ...
  return (
    <div className="bold-template bg-white text-[#1a1a1a] min-h-0 max-w-[210mm] mx-auto font-sans text-sm overflow-hidden">
      {/* Full-width dark header */}
      <header className="relative px-8 py-5 flex items-center justify-between gap-4" style={{ backgroundColor: headerBg }}>
        <div className="flex-1 text-center min-w-0 flex flex-col items-center justify-center">
          ...
          <p className={`text-[12px] uppercase tracking-wider mt-1 ${headerSubClass}`}>{jobTarget?.trim() || ph('Job title')}</p>
        </div>
        <div className={`text-right text-[12px] space-y-0.5 shrink-0 flex flex-col justify-center ${headerSubClass}`}>
